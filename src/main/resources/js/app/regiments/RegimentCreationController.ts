...
export function RegimentCreationController($scope, $state, regimentOptions, characterService, characterOptions, $q, optionselection, $uibModal, selection) {
    $q.all({characterOptions: characterOptions, regimentOptions: regimentOptions}).then(result=> {
        $scope.regimentName = "";
        var kitChoices;
         ...
         * @type {T[]}
         */
        $scope.basicWeapons = result.characterOptions.weapons.filter(function (weapon) {
         ...
         * @type {T[]}
         */
        $scope.heavyWeapons = result.characterOptions.weapons.filter(function (weapon) {
            var availability = false;
            switch (weapon.availability) {
                case "Ubiquitous":
                case "Abundant":
        ...
        kitChoices = result.regimentOptions.additionalKitChoices;
        $scope.regimentElements.homeworld['options'] = result.regimentOptions.homeworlds;
        $scope.regimentElements.homeworld['header'] = "Homeworld/Origin";
        $scope.regimentElements.homeworld['category'] = "homeworld";
        $scope.regimentElements.commander['options'] = result.regimentOptions.commandingOfficers;
        $scope.regimentElements.commander['header'] = "Commanding Officer";
        $scope.regimentElements.commander['category'] = "commander";
        $scope.regimentElements.regimentType['options'] = result.regimentOptions.regimentTypes;
        $scope.regimentElements.regimentType['header'] = "Regiment Type";
        $scope.regimentElements.regimentType['category'] = "regimentType";
        let doctrineOptions = result.regimentOptions.equipmentDoctrines.concat(result.regimentOptions.trainingDoctrines);
        $scope.regimentElements.firstSpecialDoctrine['options'] = doctrineOptions;
        ...
        $scope.regimentElements.secondSpecialDoctrine['options'] = doctrineOptions;
        ...
        function updateAvailableKitChoices() {
            $scope.kitChoices = result.regimentOptions.additionalKitChoices.map(function (choice) {
        ...
        $scope.addKitModifier = function (choice) {
            ...
            switch (choice.type) {
                ...
                {
                    let options = characterOptions.weapons.concat(
