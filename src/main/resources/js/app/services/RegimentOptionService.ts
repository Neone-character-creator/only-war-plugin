...
export class RegimentOptionService {
    ...
    constructor($resource, $q, placeholders:Promise<PlaceholderReplacement>) {
        ...
        this._homeworlds = $q.all({
            resource: $resource("/Regiment/Creation/Homeworlds.json").query().$promise,
            placeholders: transformPlaceholders
        }).then(results=> {
            return results.resource.map(results.placeholders);
        }, error);
        this._commandingOfficers = $q.all({
            resource: $resource("/Regiment/Creation/CommandingOfficers.json").query().$promise,
            placeholders: transformPlaceholders
        }).then(results=> {
            return results.resource.map(results.placeholders);
        }, error);
        this._regimentTypes = $q.all({
            resource: $resource("/Regiment/Creation/RegimentType.json").query().$promise,
            placeholders: transformPlaceholders
        }).then(results=> {
            return results.resource.map(results.placeholders)
        }, error);
        this._equipmentDoctrines = $q.all({
            resource: $resource("/Regiment/Creation/Special Equipment.json").query().$promise,
            placeholders: transformPlaceholders
        }).then(results=> {
            return results.resource.map(results.placeholders);
        }, error);
        this._trainingDoctrines = $q.all({
            resource: $resource("/Regiment/Creation/TrainingDoctrines.json").query().$promise,
            placeholders: transformPlaceholders
        }).then(results=> {
            return results.resource.map(results.placeholders)
        }, error);
        this._standardRegimentalKit = $q.all({
            resource: $resource("/Regiment/Creation/StandardRegimentalKit.json").get().$promise,
            placeholders: transformPlaceholders
        }).then(results=> {
        ...
        this._additionalKitChoices = $q.all({
            resource: $resource("/Regiment/Creation/AdditionalKitChoices.json").query().$promise,
